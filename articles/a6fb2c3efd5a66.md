...
作業には Claude Code（Fable 5）を活用しました。Cloudflare を使うのは今回が初めてで、ダッシュボードの画面や設定項目に馴染みがなく迷う場面も多かったのですが、Cloudflare の設定画面の項目や R2 まわりの仕様など不明瞭な点をその場で検証してもらいながら、帯域設計や配信構成といった設計方針を対話で固めていきました。特に「この構成で本当に大丈夫か？」と不安になる箇所をひとつずつ潰していく場面で重宝しました。
...
R2 の料金体系は以下のとおりです（2026年7月時点・Standard storage の場合）。

| 項目 | 無料枠 | 超過分 |
|---|---|---|
| ストレージ | 10GB-月 | $0.015 / GB-月 |
| Class A 操作（書き込み・一覧取得など） | 100万リクエスト/月 | $4.50 / 100万リクエスト |
| Class B 操作（読み取りなど） | 1,000万リクエスト/月 | $0.36 / 100万リクエスト |
| egress（インターネットへの下り転送） | **無料** | **無料** |

https://developers.cloudflare.com/r2/pricing/

ポイントは、**egress がどれだけ配信しても無料** であること。今回のように「大容量ファイルを繰り返し配信する」用途では、この一点だけで採用理由になります。ストレージも今回のアセット量（合計約75MB）なら無料枠に余裕で収まります。

やることは3つです。

1. バケット作成（ロケーション自動）。初回はカード登録が必要だが、上記の無料枠内なら請求は発生しない
