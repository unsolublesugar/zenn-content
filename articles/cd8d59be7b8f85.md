...
:::message alert
本記事は **2026年2月時点** の検証内容をもとに執筆しています。ツールのアップデートにより、仕様が変更されている場合があります。**Unity CLI Loop**変更後の導入手順は[こちら](https://zenn.dev/unsoluble_sugar/articles/bf666326e854d1)をご参照ください。
:::
...
:::message
追記：【Unity CLI Loop】に名称変更されました。現在はCLIの導入が推奨されています。

https://x.com/m_hatayama/status/2033336002841051389?s=20
:::
...
前述のScope(s)に `org.nuget` を追加してPackage Managerの「My Registries」からインストールします。
...
:::message
本記事は **2026年2月時点** の検証内容です。現在はツール名が **Unity CLI Loop** に変更され、CLIの導入が推奨されています。
:::

https://zenn.dev/unsoluble_sugar/articles/bf666326e854d1

また、Unity CLI Loopを使ってチュートリアルのE2Eテスト自動化基盤を構築した検証記事も書きました。AIエージェントでゲーム内操作まで自動化するイメージが気になる方は、こちらも合わせてご覧ください。
